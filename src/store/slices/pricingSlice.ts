...
export interface PricingSlice {
  ...
  updatePricingSettings: (settings: Partial<PricingSlice['pricing']['settings']>) => void;
...
export const createPricingSlice: StateCreator<any, [], [], PricingSlice> = (set, get) => ({
  pricing: {
    prices: [],
    products: [],
    stores: [],
  ...
  updatePrices: (newPrices) => set((state: PricingSlice) => {
    newPrices.forEach(priceData => {
      // Check if price already exists
      const existingIndex = state.pricing.prices.findIndex((p: PriceData) =>
  ...
  addProduct: (product) => set((state: PricingSlice) => {
  ...
  updateProduct: (id, updates) => set((state: PricingSlice) => {
    const index = state.pricing.products.findIndex((p: Product) => p.id === id);
    if (index !== -1) {
      Object.assign(state.pricing.products[index], updates, { updatedAt: new Date() });
    }
  }),

  deleteProduct: (id) => set((state: PricingSlice) => {
    state.pricing.products = state.pricing.products.filter((p: Product) => p.id !== id);
    state.pricing.prices = state.pricing.prices.filter((p: PriceData) => p.productId !== id);
    state.pricing.alerts = state.pricing.alerts.filter((a: PriceAlert) => a.productId !== id);
    state.pricing.comparisons = state.pricing.comparisons.filter((c: PriceComparison) => c.productId !== id);
  }),

  addStore: (store) => set((state: PricingSlice) => {
  ...
  updateStore: (id, updates) => set((state: PricingSlice) => {
    const index = state.pricing.stores.findIndex((s: Store) => s.id === id);
    if (index !== -1) {
      Object.assign(state.pricing.stores[index], updates, { updatedAt: new Date() });
    }
  }),

  deleteStore: (id) => set((state: PricingSlice) => {
    state.pricing.stores = state.pricing.stores.filter((s: Store) => s.id !== id);
    state.pricing.prices = state.pricing.prices.filter((p: PriceData) => p.storeId !== id);
  }),

  addPriceAlert: (alert) => set((state: PricingSlice) => {
  ...
  updatePriceAlert: (id, updates) => set((state: PricingSlice) => {
    const index = state.pricing.alerts.findIndex((a: PriceAlert) => a.id === id);
    if (index !== -1) {
      Object.assign(state.pricing.alerts[index], updates);
    }
  }),

  deletePriceAlert: (id) => set((state: PricingSlice) => {
    state.pricing.alerts = state.pricing.alerts.filter((a: PriceAlert) => a.id !== id);
  }),

  triggerPriceAlert: (id) => set((state: PricingSlice) => {
    const alert = state.pricing.alerts.find((a: PriceAlert) => a.id === id);
    if (alert) {
  ...
  compareProductPrices: (productId) => set((state: PricingSlice) => {
    const product = state.pricing.products.find((p: Product) => p.id === productId);
    if (!product) return;

    const productPrices = state.pricing.prices
      .filter((p: PriceData) => p.productId === productId && p.active)
      .map((price: PriceData) => ({
        ...price,
        store: state.pricing.stores.find((s: Store) => s.id === price.storeId)!
      }))
      .filter((p: any) => p.store);
    ...
    const existingIndex = state.pricing.comparisons.findIndex((c: PriceComparison) => c.productId === productId);
    if (existingIndex !== -1) {
  ...
  updateStoreComparisons: (items) => set((state: PricingSlice) => {
    const storeComparisons: StoreComparison[] = [];

    state.pricing.stores.filter((store: Store) => store.active).forEach((store: Store) => {
      let totalPrice = 0;
      let totalItems = 0;
      ...
      items.forEach(item => {
        const product = state.pricing.products.find((p: Product) =>
          p.normalizedName.includes(item.name.toLowerCase()) ||
          p.name.toLowerCase().includes(item.name.toLowerCase())
        );

        if (product) {
          const price = state.pricing.prices.find((p: PriceData) =>
  ...
  findDeals: (category, minDiscount = 10) => set((state: PricingSlice) => {
    const deals: typeof state.pricing.deals = [];

    // Find products with significant price variations
    state.pricing.products.forEach((product: Product) => {
      if (category && product.category !== category) return;

      const productPrices = state.pricing.prices
        .filter((p: PriceData) => p.productId === product.id && p.active);
  ...
  updatePricingSettings: (settings) => set((state: PricingSlice) => {
    Object.assign(state.pricing.settings, settings);
  }),

  setPricingLoading: (loading) => set((state: PricingSlice) => {
    state.pricing.isLoading = loading;
  }),

  syncPrices: () => set((state: PricingSlice) => {
