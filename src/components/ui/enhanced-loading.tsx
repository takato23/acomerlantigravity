...
export function EnhancedLoading({
  ...
  const baseClasses = {
    ...
    inline: 'w-full p-6 bg-white rounded-lg shadow-lg border border-slate-200',
    fullscreen: 'fixed inset-0 bg-white flex items-center justify-center z-50'
  };

  const contentClasses = {
    modal: 'bg-white rounded-xl shadow-2xl max-w-lg w-full mx-4 p-8 text-slate-900',
    inline: 'w-full text-slate-900',
    fullscreen: 'max-w-2xl w-full mx-4 bg-white text-slate-900'
  };
      ...
      >
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    <h4 className={`font-medium ${
                      ...
                      stage.status === 'active' ? 'text-slate-800' :
                  ...
                  <p className={`text-xs mt-1 ${
                    ...
                    stage.status === 'active' ? 'text-slate-600' :
        ...
        <div className="mt-6 p-4 bg-slate-50 rounded-lg">
          <div className="flex items-center mb-2">
            ...
            <span className="text-sm font-medium text-slate-700">¿Sabías que?</span>
          </div>
...
export function SkeletonLoader({ 
  ...
  if (variant === 'card') {
    return (
      <div className={`${className} bg-white rounded-lg shadow-sm border border-slate-200`}>
        <div className={`h-48 bg-slate-200 rounded-t-lg ${animationClass}`} />
        <div className="p-4 space-y-3">
          <div className={`h-4 bg-slate-200 rounded ${animationClass}`} />
          <div className={`h-4 bg-slate-200 rounded w-3/4 ${animationClass}`} />
          <div className={`h-4 bg-slate-200 rounded w-1/2 ${animationClass}`} />
        </div>
      </div>
    );
  }
  ...
  if (variant === 'recipe') {
    return (
      <div className={`${className} bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden`}>
        <div className={`h-32 bg-slate-200 ${animationClass}`} />
        <div className="p-4">
          <div className={`h-6 bg-slate-200 rounded mb-3 ${animationClass}`} />
          <div className={`h-4 bg-slate-200 rounded mb-2 ${animationClass}`} />
          <div className={`h-4 bg-slate-200 rounded w-2/3 ${animationClass}`} />
        </div>
      </div>
    );
  }
  ...
  if (variant === 'calendar') {
    return (
      <div className={`${className} bg-white rounded-lg shadow-sm border border-slate-200 p-4`}>
        <div className={`h-6 bg-slate-200 rounded mb-4 ${animationClass}`} />
        <div className="space-y-3">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className={`h-12 bg-slate-200 rounded ${animationClass}`} />
          ))}
        </div>
      </div>
    );
