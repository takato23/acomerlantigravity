...
interface RecipeAvailability<T extends RecipeLike> {
  recipe: T;
 ...
 */
export function useRecipeAvailability<T extends RecipeLike>(recipes: T[]) {
  ...
  const pantryIngredientNames = useMemo(() => {
    ...
    pantryItems.forEach(item => {
      if (item.ingredient_name) {
        ...
        names.add(item.ingredient_name.toLowerCase().trim());

        // Also add common variations
        const normalized = item.ingredient_name
          .toLowerCase()
  ...
  const recipesWithAvailability = useMemo((): RecipeAvailability<T>[] => {
  ...
  const getRecipeAvailability = (recipeId: string): RecipeAvailability<T> | undefined => {
