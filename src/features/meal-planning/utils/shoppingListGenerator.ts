...
const normalizeCategory = (category: string): IngredientCategory => {
  return INGREDIENT_CATEGORY_ORDER.includes(category as IngredientCategory)
    ...
    slot.recipe.ingredients.forEach(ingredient => {
      ...
      if (ingredientMap.has(key)) {
        ...
        if (existing.unit === ingredient.unit) {
          existing.totalAmount += amount * slot.servings;
        } else {
          // Different units - create a note about the additional quantity
          const additionalNote = `${amount * slot.servings} ${ingredient.unit}`;
        ...
        ingredientMap.set(key, {
          ...
          totalAmount: amount * slot.servings,
...
function groupByCategory(items: ShoppingListItem[]): ShoppingListCategory[] {
  ...
  INGREDIENT_CATEGORY_ORDER.forEach(category => {
...
function calculateEstimatedTotal(items: ShoppingListItem[]): number {
  ...
  return items.reduce((total, item) => {
    ...
    const basePrice = getBasePriceByCategory(normalizeCategory(item.category));
...
export function suggestPackageSizes(item: ShoppingListItem): ShoppingListItem['packageInfo'] {
  ...
  const defaultSizes = [{ amount: 1, unit: 'unidad' }];
  const packageSizes: Partial<Record<IngredientCategory, Array<{ amount: number; unit: string }>>> = {
    verduleria: [
      { amount: 1, unit: 'kg' },
      { amount: 500, unit: 'g' },
    ...
    limpieza: defaultSizes,
    otros: defaultSizes
  };
  
  const availableSizes = packageSizes[normalizeCategory(item.category)] || defaultSizes;
